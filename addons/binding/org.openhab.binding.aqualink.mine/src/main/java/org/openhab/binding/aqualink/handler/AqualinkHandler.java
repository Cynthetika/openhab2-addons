...
public class AqualinkHandler extends BaseThingHandler implements AqualinkStatusListener {
        ...
        if (command == RefreshType.REFRESH) {
		    ...
		    break;
                case CHANNEL_POOL_TEMPERATURE:
                    updateState(CHANNEL_POOL_TEMPERATURE, new DecimalType(lastStatus.getAir_temp()));
	            break;
                case CHANNEL_SPA_TEMPERATURE:
                    updateState(CHANNEL_SPA_TEMPERATURE, new DecimalType(lastStatus.getAir_temp()));
	            break;
            }
        } else 
     	{
    ...
    public void aqualinkLEDStatusChanged(AqualinkLEDStatus aqualinkLEDStatus) {
        ...
        if (!aqualinkLEDStatus.getPool_heater().equals(lastLEDStatus.getPool_heater())) {
            updateState(CHANNEL_POOL_HEATER, (aqualinkLEDStatus.getPool_heater().equals("ON")) ? OnOffType.ON : OnOffType.OFF);
        ...
        if (!aqualinkLEDStatus.getSpa_heater().equals(lastLEDStatus.getSpa_heater())) {
            updateState(CHANNEL_SPA_HEATER, (aqualinkLEDStatus.getSpa_heater().equals("ON")) ? OnOffType.ON : OnOffType.OFF);
